...
_successExec =
{
	...
	_randomBox = ["mission_Launchers1","mission_Weapon1","mission_Snipers1"] call BIS_fnc_selectRandom;
	...
	while {(position _box1) select 2 > 5 && attachedTo _box1 == _para} do
	{
	...
	waitUntil {((((position _box1) select 2) < 1) || (isNil "_para"))};
	detach _box1;
