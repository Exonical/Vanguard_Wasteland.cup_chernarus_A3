...
#define CLEANUP_INTERVAL (7*60) // Interval to run the cleanup
#define ITEM_CLEANUP_TIME (7*60) // Dropped player items cleanup time
...
#define DEBRIS_CLEANUP_TIME (3*60) // Vehicle crash crater/debris cleanup time (actual vehicle wreck cleanup is handled through description.ext parameters)
...
if (!isServer) then {
	//no_log "WASTELAND HEADLESS - Object cleanup enabled";
};
...
while {true} do
{
	...
	//no_log format ["SERVER CLEANUP: Deleted %1 expired objects", _delQtyO];

	_delQtyG = 0;
	_groupCleanup forEach allGroups;

	//no_log format ["SERVER CLEANUP: Deleted %1 empty groups", _delQtyG];
};
