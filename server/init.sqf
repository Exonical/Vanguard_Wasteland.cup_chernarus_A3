...
//no_log "WASTELAND SERVER - Server Compile Finished";
...
if (_hcObjSavingOn) then
{
	...
	{
		//no_log "WASTELAND HEADLESS - Object saving enabled";
...
if (_playerSavingOn || _objectSavingOn || _vehicleSavingOn || _mineSavingOn || _timeSavingOn || _weatherSavingOn) then
{
	...
	if ((_savingMethod == "extDB2") || (_savingMethod isEqualTo "extDB3"))  then { _savingMethod = "extDB" };

	// extDB
	if (_savingMethod isEqualTo "extDB") then
	{
		_version = "extDB3" callExtension "9:VERSION";
		diag_log format ["[Aryx] extDB3 v%1", _version];
		...
		else
		{
			...
			{
				diag_log "[INFO] ███ extDB3 NOT FOUND! Make sure extDB3.dll (Windows) or extDB3.so (Linux) and extdb-conf.ini are in the same directory as arma3server, and that you are using the -filePatching parameter";
			};
	...
	if (_savingMethod isEqualTo "iniDBI") then { _savingMethod = "iniDB" };

	// iniDB
	if (_savingMethod isEqualTo "iniDB") then
	{
	...
	if (_savingMethod isEqualTo "profileNamespace") then { _savingMethod = "profile" };

	// profileNamespace
	if (_savingMethod isEqualTo "profile") then
	{
...
{
	//no_log "[INFO] A3W loot spawning is ENABLED";
...
{
	//no_log "WASTELAND SERVER - Initializing Server Spawning";
...
{
	//no_log "[INFO] A3W territory capturing is ENABLED";
	[] execVM "territory\server\monitorTerritories.sqf";
};
/*
else
{
	//no_log "[INFO] A3W territory capturing is DISABLED";
};*/
...
{
	//no_log "WASTELAND SERVER - Initializing Missions";
