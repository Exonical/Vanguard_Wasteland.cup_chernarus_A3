/*
	...
	Description:
	out of bounds checks for player in a ship.
...
_maxHeight = 1500; // max water vehicle height (same height as helicopter, as some water vehicles can be slingloaded)
...
if (isServer) then
{
	...
	_mapSizeSquare = 15360;
	_mapCenterPos = [(_mapSizeSquare / 2),(_mapSizeSquare / 2)]; 
	_mkrInBounds = createMarker ["playableAreaShip",_mapCenterPos]; 
	_mkrInBounds setMarkerColor "ColorRed"; 
	_mkrInBounds setMarkerShape "RECTANGLE"; 
	_mkrInBounds setMarkerBrush "BORDER"; 
	_mkrInBounds setMarkerAlpha 1; 
	_mkrInBounds setMarkerSize [(_mapSizeSquare / 2),(_mapSizeSquare / 2)];
};
...
if (hasInterface) then
{
	...
	[_maxTime,_maxHeight] spawn
	{
		...
		while {_loopForever} do
		{
			...
			waitUntil {uiSleep 1; ((objectParent player) isKindOf "Ship") && (!((objectParent player) inArea "playableAreaShip") || ((getPos (objectParent player) select 2) > _maxHeight))};
			///////////////////////////////////////////////////////////////////////////////////////
			_secondsTimer = _maxTime;
			_secondsText = "";
			while {!((objectParent player) inArea "playableAreaShip") || ((getPos (objectParent player) select 2) > _maxHeight)} do
			{
			...
			if !((objectParent player) isEqualTo objNull) then
			{
				...
				{
					_text = "YOU AND YOUR WATER VEHICLE ARE BACK INSIDE THE PLAYABLE AREA";
					[_text, 10] call mf_notify_client;
				}
				else
				{
					_text = "YOU WERE KILLED AND YOUR WATER VEHICLE WAS DESTROYED DUE TO BEING OUTSIDE OF THE PLAYABLE AREA FOR TOO LONG";
					[_text, 10] call mf_notify_client;
				};
			};
			///////////////////////////////////////////////////////////////////////////////////////
