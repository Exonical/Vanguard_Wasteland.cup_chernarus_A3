...
"pvar_bountyProcessing" addPublicVariableEventHandler {
	...
	switch (toLower _type) do {
		case "addbounty": {
			...
			if (_cost > 0) then {
				if (!isPlayer _sender || !isPlayer _recipient || _sender isEqualTo _recipient) exitWith {}; // invalid sender or recipient
		...
		case "rewardbounty": {
			...
			if (_reward > 0) then {
			if (!isPlayer _outlaw || !isPlayer _bountyHunter || _outlaw isEqualTo _bountyHunter) exitWith {}; // invalid sender or recipient
