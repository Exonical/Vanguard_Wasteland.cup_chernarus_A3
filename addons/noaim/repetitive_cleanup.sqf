...
GVAR(deleteClassesConfig) = [
	...
	[M* 5*60, ["NonSteerable_Parachute_F","Steerable_Parachute_F","Land_FoodSack_01_full_white_idap_F","Land_CanisterFuel_F","Land_CanisterOil_F","Chemlight_blue","Chemlight_green","Chemlight_red","Chemlight_yellow","WeaponHolder","GroundWeaponHolder","WeaponHolderSimulated","Ruins","#destructioneffects","CraterLong_small","CraterLong","SmokeShellBlue","SmokeShellGreen","SmokeShellOrange","SmokeShellRed","SmokeShell","SmokeShellYellow"]],
	[M* 10*60, ["BagFence_base_F","#dynamicsound","#track","#particlesource"]]
];

GVAR(resetTimeIfPlayerIsWithin) = 80; // how far away from object player needs to be so it can delete
	...
	call _deleteMarkedIndexes;
	{
		...
		if(IS_SANE(_object)) then {
			...
			if((GVAR(timesWhenToCleanup) select _objectIndex) < time) then {
				...
				//diag_log format ["xxxxxxxxxxxxxxx  DEBUG: _object %1  xxxxxxxxxxxxxxxxxx", _object];
			};
		} else {
			[_objectIndex] call _markArraysForCleanupAt;
		};
