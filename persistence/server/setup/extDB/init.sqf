 ...
 _return = if (isNil {uiNamespace getVariable "A3W_extDB_databaseID"} || !_lock) then
{
	...
	if (isNil {uiNamespace getVariable "A3W_extDB_databaseID"}) then
	{
		...
		if (_result select 0 isEqualTo 0) exitWith { diag_log format ["[extDB3] ███ Database error! %1", _result]; false };
	};
	...
	if (_result select 0 isEqualTo 0) exitWith { diag_log format ["[extDB3] ███ SQL_CUSTOM Protocol error! %1", _result]; false };
	...
	if (["A3W_extDB_Misc"] call isConfigOn) then
	{
		...
		if (_result select 0 isEqualTo 0) exitWith { diag_log format ["[extDB3] ███ MISC Protocol error! %1", _result]; false };
