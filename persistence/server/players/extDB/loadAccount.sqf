...
params ["_UID", "_player"];
private ["_result", "_result2", "_data", "_location", "_dataTemp", "_ghostingTimer", "_secs", "_columns", "_pvar", "_pvarG"];

_donatorLevel = 0;
_memberLevel = 0;
private _moneySaving = ["A3W_moneySaving"] call isConfigOn;
private _customLoadoutEnabled = ["A3W_customLoadoutEnabled"] call isConfigOn;
...
_result2 = ["getPlayerMemberLevel:" + _UID, 2] call extDB_Database_async;

if (count _result > 0) then {
	_donatorLevel = _result select 0;
	_memberLevel = _result2 select 0;
};
...
else
{
	...
	_data pushBack ["MemberLevel", _memberLevel];
};
...
_result = [["getPlayerStatusXMap", _UID], 2] call extDB_Database_async;
...
if (["A3W_privateParking"] call isConfigOn) then
{
	...
	_result = [["getParkedVehiclesMOD", _UID, _environment, _columns joinString ","], 2, true] call extDB_Database_async;
...
if (["A3W_privateStorage"] call isConfigOn) then
{
	...
	_result = [["getPlayerStorageXMapMOD", _UID, _environment, _columns joinString ","], 2] call extDB_Database_async;
