...
A3W_fnc_savePlayerData =
{
	...
	if(_player getVariable ["bountyKills", []] isEqualTo 0) then {
		_player setVariable ["bountyKills", [], true];
	};
		...
		[[_player, _UID, _data],
		{
...

			{
				...
				switch (_var) do
				{
					...
					case "MemberLevel":  { _player setVariable ["memberLevel", _val, true] };
				};
			} forEach _data;
			
			if (!isNil "bountyKillsExpired") then {
