...
diag_log "[MAP CLEANUP] SCRIPT LOADED";
...
private _ttdBodies = M* 3*60; // seconds to delete dead bodies (0 means don't delete)
private _ttdVehiclesDead = M* 3*60; // seconds to delete dead vehicles (0 means don't delete)
private _ttdVehiclesImmobile = M* 8*60; // seconds to delete immobile vehicles (0 means don't delete)

GVAR(deleteClassesConfig) = [
	// player placed or created objects that you probably want to delete once they are far enough
	[M* 15*60, ["Land_Sleeping_bag_folded_F", "Land_CanisterFuel_F", "Land_CanisterOil_F", "Land_Ground_sheet_folded_OPFOR_F", "Land_PaperBox_01_small_closed_brown_F"]],
	[M* 10*60, ["BagFence_base_F","CraterLong_small","CraterLong","SmokeShell"]],
	[M* 5*60, ["WeaponHolder","GroundWeaponHolder","WeaponHolderSimulated"]],
	[M* 2*60, ["#dynamicsound","#destructioneffects","#track","#particlesource"]]
	//[M* 90, ["Land_Suitcase_F"]]
];

GVAR(resetTimeIfPlayerIsWithin) = 25; // how far away from object player needs to be so it can delete
...

  {
    _object = _x;
	    ...
	    if(_timeToDelete>0) then {
				_cleanupTime = _object getVariable["A3W_Cleanup", -1];
				if (_cleanupTime > 0) then {
					[_object, _cleanupTime, true, false] call GVAR(addToCleanup)
				} else {
					{
